...
from integrations import TTSClient, HomeAssistantClient, WhatsAppClient
...
async def execute_rule_pipeline(rule: Rule, sensor: Sensor, media_paths: List[str], media_type: str, session: Session):
    ...
    try:
        ...
        full_logic_prompt = f"""
        Context: Room {sensor.room_name}, Time(24hr format) {datetime.now().strftime('%H:%M')}.
        ...
        await tts_client.generate_audio(feedback_message, audio_path, voice="en-US-AvaMultilingualNeural", speed=0.85)
        audio_url = await asyncio.to_thread(minio_client.upload_file, audio_path, f"{request_id}.mp3")
        if audio_url:
            await ha_client.play_audio(audio_url)
        asyncio.to_thread(minio_client.delete_object, f"{request_id}.mp3")
        time.sleep(13)
        # Generate Audio in Tamil
        request_id_ta = str(uuid.uuid4())
        audio_path = f"/tmp/{request_id_ta}.mp3"
        await tts_client.generate_audio(feedback_message_ta, audio_path, voice="ta-IN-PallaviNeural", speed=0.85)
        audio_url = await asyncio.to_thread(minio_client.upload_file, audio_path, f"{request_id_ta}.mp3")
        if audio_url:
            await ha_client.play_audio(audio_url)
        asyncio.to_thread(minio_client.delete_object, f"{request_id_ta}.mp3")
